...
def sv_definitions(path: Path) -> dict[str, int]:
    values: dict[str, int] = {}
    pattern = r"^`define PS2_([A-Z0-9_]+)\s+(?:8|12|32)'h([0-9A-Fa-f_]+)$"
...
def c_definitions(path: Path) -> dict[str, int]:
    values: dict[str, int] = {}
    pattern = r"^#define PS2_([A-Z0-9_]+)\s+UINT32_C\(0x([0-9A-Fa-f]+)\)$"
    for match in re.finditer(pattern, path.read_text(encoding="utf-8"), re.MULTILINE):
...
def main() -> int:
    rtl = sv_definitions(ROOT / "rtl" / "ps2_define.svh")
    c_header = c_definitions(ROOT / "sw" / "include" / "ps2_regs.h")
    ...
    required_scalars = {
        "CTRL_ENABLE_MASK",
        "FIFO_CTRL_RX_FLUSH_MASK",
        "FIFO_CTRL_TX_FLUSH_MASK",
        "COMMAND_ABORT_MASK",
        "COMMAND_SOFT_RESET_MASK",
        "ERROR_VALID_MASK",
        "INTR_VALID_MASK",
        "IP_ID_VALUE",
        "IP_VERSION_VALUE",
